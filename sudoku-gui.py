...
from sudoku1 import valid, solve, find_empty
    ...
    def visualSolve(self, wrong, time):
        ...
        for nums in range(9):
            if valid(self.board, (empty[0],empty[1]), nums+1):
                ...
                pygame.time.delay(10) #show tiles at a specified rate
...
def main():
    ...
    wrong = 0
    board = Board(screen)
    min_moves=count_min_moves(board)
...
def suggest(count,min_moves,difficulty):
    ...
    if difficulty == "Hard":
        if(count<=min_moves+5):
            print("You have reached God level ")
            return "Hard"
        elif(count<=min_moves+15):
            print("Need more practice,Keep playing")
            return "Hard"
...
while True:
    main()
    difficulty = suggest(moves,min_moves,s)
    s = menu(s)
    if(s==-1):
        break
pygame.quit()